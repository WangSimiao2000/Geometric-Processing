...
public:
    ...
    std::vector<Cartesian3> oriVertices;
    std::vector<Cartesian3> oriColours;
    std::vector<std::pair<unsigned int,unsigned int>> halfEdges;
    std::vector<std::vector<unsigned int>>   neighborPoints;
    std::vector<unsigned int> boundaryEdges;
    std::vector<unsigned int> boundaryPoints;

    void calculateHalfEdgesAndPoints();
    void findBoundaryPointAndSortThem();
    void createSquareTexture();
    void calculateAndDoFloater();
    bool checkIsBoundaryPoint(unsigned int v);
    void calculateNormals();
    void textureAndNormalMapSynthesis();
}; // class AttributedObject
